...
void *fungsi (int socketfd){
	...
	for(;;){
		...
		if(strncmp("cap", dariClient, 3) == 0){		
			...
			strcpy(buff, "graph_args --base 1024 -l 0 --upper-limit ");
			FILE *fp = popen("free | awk 'FNR == 2 {print $2}'", "r");
			if (fp == NULL) {
...
int main(){
	...
	if(socketfd == -1){
		...
		//do nothing. socket berhasil dibuat
	}
	...
	if((bind(socketfd, (SA*)&serveraddr, sizeof(serveraddr))) != 0){
		...
		//do nothing. socket binding berhasil
	}
